...
def solution1(amphipods):
    ...
    def is_new_move(move: Move):
        gamestate = (tuple(move.board[0]), tuple(move.board[1]))
        return gamestate not in visited or visited[gamestate] > move.energy
    ...
    while queue:
        ...
        for hallway_idx, apidx in enumerate(hallway):
            ...
            for room_idx in amphipod.rooms:
                move = make_hallway_to_room_move(amphipods, apidx, energy, hallway, hallway_idx, room_idx, rooms)
                if move and is_new_move(move):
        ...
        for room_idx, apidx in enumerate(rooms):
            ...
            for i in range(4):
                ...
                move = make_room_to_room_move(amphipods, apidx, energy, hallway, room_idx, home, rooms)
                if move and is_new_move(move):
                    heapq.heappush(queue, move)
                    break
            else:
                for hallway_idx in range(HALLWAY_COUNT):
                    move = make_room_to_hallway_move(amphipods, apidx, energy, hallway, hallway_idx, room_idx, rooms)
                    if move and is_new_move(move):
                        heapq.heappush(queue, move)
    return best_solution


def make_room_to_room_move(amphipods, apidx, energy, hallway, from_room, to_room, rooms):
    ...
    score = completion_score(new_board[1], amphipods)

    return Move(-score, sum(e), next(seqnum), new_board)


def make_room_to_hallway_move(amphipods, apidx, energy, hallway, hallway_idx, room_idx, rooms):
    ...
    e = energy + cost(amphipods[apidx], room_idx, hallway_idx)
    score = completion_score(new_board[1], amphipods)
    return Move(-score, e, next(seqnum), new_board)


def make_hallway_to_room_move(amphipods, apidx, energy, hallway, hallway_idx, room_idx, rooms):
    amphipod = amphipods[apidx]
